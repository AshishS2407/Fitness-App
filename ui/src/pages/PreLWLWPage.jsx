...
const LightWeightLossPage = ({ userType }) => { // Accept userType as a prop
    const [videos, setVideos] = useState([]);

    useEffect(() => {
        const fetchVideosByCategory = async () => {
            try {
                const response = await axios.get('http://localhost:5000/workouts?category=light weight loss');
...
export default LightWeightLossPage;
